...
class CreatePolygons(Extract):
    ...
    def create_poly_and_multipolygons(self, schema='osm'):
        ...
        with Connection(login=self.login) as conn:
            self.conn = conn
            if self.conn.relation_exists('simple_polys', schema):
                self.truncate_table('simple_polys', schema)
            else:
                sql = f'''
            ...
            if self.conn.relation_exists('polygon_with_holes', schema):
                self.truncate_table('polygon_with_holes', schema)
            else:
                sql = f'''
            ...
            if self.conn.relation_exists('multi_polygons', schema):
                self.truncate_table('multi_polygons', schema)
            else:
                sql = f'''
